...
/*
хранить в объекте calendar сведения, используется ls или fb
и он передается в storage

переделать ls через класс
 */




// function serialProcess(list, processingCallback, resultCallback) {
//   let result = [];
//   list.reduce((promise, element, index, array) => {
//       return promise.then(() =>
//         new Promise((resolve, reject) => {
//           processingCallback(element, index, array, (x) => {
//             result.push(x);
//             resolve(1);
//           })
//         })
//       )
//     }, Promise.resolve())
//     .then(() => resultCallback(result));
// }


/**
 * сайт погоды
 * роуты об авторе, главная, автор, координаты (динамический), город
 * 
 * каждый  роут имеет компонент для каждой страницы слайд 13
 * 
 * компоненты: about, author, и для поиска (координаты и город (запрос на гугл, получить координаты и опять же через api получить координаты))
 * 
 * сервисы с одним интерфейсом, которые умеют делать запросы (XHR/fetch с одинаковым интерфейсом, которые умеют читать урл)(
 * запрос погоды - асинхр
 * запрос координат - асинхр
 * )
 * сервис асинхр с хранилищем
 * 
 * 
 * EventBus - для связывания частей приложения
 * 
 * 1 компонент - 1 файл
 * 
 * миникомпоненты
 * History - eventBus и хранилище
 * 
 * Favourites - звездочка на карте
 * eventBus и хранилище
 * 
 * виджет погоды (карта + eventBus + сервис для работы с погодой)
 * вижет карты (api, eventBus, для событий изм-я координат и избр)
 * 
 * меню (eventBus Для fetch / xhr)
 * поиск (eventBus)
 * 
 * проверять, что если карта уже есть, ее не перерисовывать (чтоб не было моргания при перемещении)
 */